...
def remove_entry(index):
    ...
    if 0 <= index < len(captured_data):
        removed_plate_number = captured_data.pop(index)["plate_number"]
        save_captured_data(captured_data)
        
        # Remove the number plate from the predefined set
        if removed_plate_number in predefined_number_plates:
            predefined_number_plates.remove(removed_plate_number)
